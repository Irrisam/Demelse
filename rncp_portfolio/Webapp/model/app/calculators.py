...
def value_getter(user_id, data_type='full', dominant_category_check = False):
    ...
    if data_type == 'full':
        data_categories = [['ETAB', 'USER'],['ETAB', 'CONTENT'], ['SERVICE', 'USER'],['SERVICE', 'CONTENT'], ['RYTHME', 'CONTENT']]
    score_values = []
    dominant_recommendation_scores = {'user_score': 0, 'content_score': 0}
    for recommendation_type in range(len(data_categories)):
        dataset_path = '{}_{}_TRAINING.csv'.format(data_categories[recommendation_type][1], data_categories[recommendation_type][0])
        data_type_fetch = str(data_categories[recommendation_type][1]) + '_' + str(data_categories[recommendation_type][0])
    ...
    summed_up_scores_list = [(key, value) for key, value in aggregated_scores.items()]
...
def mission_trios_selector(user_id, result_limit, trio_index, dominant_category_check = False):
    ...
    try:
        returned_score_trios, full_trios = generate_top_combinations(user_id, result_limit, dominant_category_check)
